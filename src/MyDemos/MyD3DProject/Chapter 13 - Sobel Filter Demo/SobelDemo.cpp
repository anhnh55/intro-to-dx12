...
SobelDemo::SobelDemo(HINSTANCE hInstance)
	: D3DApp(hInstance)
{
	mMainWndCaption = L"Sobel Demo";
}
...
void SobelDemo::CreateRtvAndDsvDescriptorHeaps()
{
...
void SobelDemo::Update(const GameTimer& gt)
{
	OnKeyboardInput(gt);
	UpdateCamera(gt);
...
void SobelDemo::Draw(const GameTimer& gt)
{
...
//	mCommandList->ClearRenderTargetView(CurrentBackBufferView(), Colors::LightSteelBlue, 0, nullptr);
	...
	//execute compute shader sobel
	mSobelFilter->Execute(mCommandList.Get(), mPostProcessRootSignature.Get(),
		mPSOs["sobel"].Get(), mOffscreenRT->Srv());

	// Prepare to copy sobel output to the back buffer.
	mCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(),
		D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_COPY_DEST));

	mCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(mSobelFilter->Output(),
		D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_COPY_SOURCE));

	mCommandList->CopyResource(CurrentBackBuffer(), mSobelFilter->Output());
...
void SobelDemo::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
...
void SobelDemo::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void SobelDemo::OnMouseUp(WPARAM btnState, int x, int y)
{
	//release mouse capture when mouse is released
	ReleaseCapture();
}

void SobelDemo::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void SobelDemo::OnKeyboardInput(const GameTimer& gt)
{
...
void SobelDemo::UpdateCamera(const GameTimer& gt)
{
...
void SobelDemo::UpdateWaves(const GameTimer& gt)
{
...
void SobelDemo::BuildInputLayout()
{
...
void SobelDemo::BuildRootSignature()
{
...
void SobelDemo::BuildPostProcessRootSignature()
{
...
void SobelDemo::BuildFrameResources()
{
...
void SobelDemo::UpdateObjectCBs(const GameTimer& gt)
{
...
void SobelDemo::UpdateMainPassCB(const GameTimer& gt)
{
...
void SobelDemo::UpdateMaterialCBs(const GameTimer& gt)
{
...
float SobelDemo::GetHillsHeight(float x, float z)
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 SobelDemo::GetHillsNormal(float x, float z) const
{
	// n = (-df/dx, 1, -df/dz)
...
void SobelDemo::BuildDescriptorHeaps()
{
...
void SobelDemo::BuildLandGeometry()
{
...
void SobelDemo::BuildBoxGeometry()
{
	GeometryGenerator geoGen;
...
void SobelDemo::BuildWavesGeometryBuffers()
{
...
void SobelDemo::BuildRenderItems()
{
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> SobelDemo::GetStaticSamplers()
{
...
void SobelDemo::AnimateMaterials(const GameTimer& gt)
{

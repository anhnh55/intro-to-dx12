...
GeometryShaderDemo::GeometryShaderDemo(HINSTANCE hInstance)
	: D3DApp(hInstance)
{
	mMainWndCaption = L"Geometry Shader Demo";
}
...
bool GeometryShaderDemo::Initialize()
{
...
void GeometryShaderDemo::OnResize()
{
...
void GeometryShaderDemo::Update(const GameTimer& gt)
{
	OnKeyboardInput(gt);
	UpdateCamera(gt);
...
void GeometryShaderDemo::Draw(const GameTimer& gt)
{
...
void GeometryShaderDemo::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
...
void GeometryShaderDemo::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void GeometryShaderDemo::OnMouseUp(WPARAM btnState, int x, int y)
{
	//release mouse capture when mouse is released
	ReleaseCapture();
}

void GeometryShaderDemo::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void GeometryShaderDemo::OnKeyboardInput(const GameTimer& gt)
{
...
void GeometryShaderDemo::UpdateCamera(const GameTimer& gt)
{
...
void GeometryShaderDemo::UpdateWaves(const GameTimer& gt)
{
...
void GeometryShaderDemo::BuildInputLayout()
{
...
void GeometryShaderDemo::BuildRootSignature()
{
...
void GeometryShaderDemo::CompileShaders()
{
...
void GeometryShaderDemo::BuildPSOs()
{
...
void GeometryShaderDemo::BuildFrameResources()
{
...
void GeometryShaderDemo::UpdateObjectCBs(const GameTimer& gt)
{
...
void GeometryShaderDemo::UpdateMainPassCB(const GameTimer& gt)
{
	...
	XMFLOAT3 baseLightStrength = { 1.0f, 1.0f, 1.0f };
...
void GeometryShaderDemo::UpdateMaterialCBs(const GameTimer& gt)
{
...
float GeometryShaderDemo::GetHillsHeight(float x, float z)
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 GeometryShaderDemo::GetHillsNormal(float x, float z) const
{
	// n = (-df/dx, 1, -df/dz)
	//based on calculus tangent vectors (book 8.14.2)
	XMFLOAT3 n(
...
void GeometryShaderDemo::LoadTextures()
{
	...
	srvHeapDesc.NumDescriptors = 4; //because we load 4 textures
	...
	auto grassTexResource = mTextures["grassTex"]->Resource;
	auto waterTexResource = mTextures["waterTex"]->Resource;
	auto fenceTexResource = mTextures["fenceTex"]->Resource;
	auto treeArrayTexResource = mTextures["treeArrayTex"]->Resource;

	D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
	srvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING; //which will not reorder the componentsand just return the data in the order it is stored in the texture resource
	srvDesc.Format = grassTexResource->GetDesc().Format;
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	srvDesc.Texture2D.MostDetailedMip = 0;
	srvDesc.Texture2D.MipLevels = -1; //This field, along with MostDetailedMip allows us to
	...
	md3dDevice->CreateShaderResourceView(grassTexResource.Get(), &srvDesc, descriptorHandle);

	// next descriptor
	descriptorHandle.Offset(1, mCbvSrvDescriptorSize);

	srvDesc.Format = waterTexResource->GetDesc().Format;
	md3dDevice->CreateShaderResourceView(waterTexResource.Get(), &srvDesc, descriptorHandle);

	// next descriptor
	descriptorHandle.Offset(1, mCbvSrvDescriptorSize);

	srvDesc.Format = fenceTexResource->GetDesc().Format;
	md3dDevice->CreateShaderResourceView(fenceTexResource.Get(), &srvDesc, descriptorHandle);

	// next descriptor
	descriptorHandle.Offset(1, mCbvSrvDescriptorSize);
	auto desc = treeArrayTexResource->GetDesc();
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DARRAY;
	srvDesc.Format = treeArrayTexResource->GetDesc().Format;
	srvDesc.Texture2DArray.MostDetailedMip = 0;
	srvDesc.Texture2DArray.MipLevels = -1;
	srvDesc.Texture2DArray.FirstArraySlice = 0;
	srvDesc.Texture2DArray.ArraySize = treeArrayTexResource->GetDesc().DepthOrArraySize;
	md3dDevice->CreateShaderResourceView(treeArrayTexResource.Get(), &srvDesc, descriptorHandle);
}

void GeometryShaderDemo::BuildLandGeometry()
{
...
void GeometryShaderDemo::BuildBoxGeometry()
{
	GeometryGenerator geoGen;
...
void GeometryShaderDemo::BuildTreeSpritesGeometry()
{
...
void GeometryShaderDemo::BuildMaterials()
{
	...
	water->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 0.5f);//texture file for water has alpha 1 so we set 0.5 here to demo blending
...
void GeometryShaderDemo::BuildWavesGeometryBuffers()
{
...
void GeometryShaderDemo::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&gridRitem->TexTransform, XMMatrixScaling(5.0f, 5.0f, 1.0f));//to tile the texture
	...
	mRitemLayer[(int)RenderLayer::AlphaTested].push_back(crateRitem.get());
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> GeometryShaderDemo::GetStaticSamplers()
{
...
void GeometryShaderDemo::AnimateMaterials(const GameTimer& gt)
{
	// Scroll the water material texture coordinates.
	auto waterMat = mMaterials["water"].get();

	//Todo CH9:
	//Note that we set transformatrix at 3,0 and 3,1 because later the matrix will be transposed before sent to GPU
	//(hlsl matrix is column major while directX matrix is row major)
